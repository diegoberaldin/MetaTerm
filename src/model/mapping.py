...
This module contains all transfer object definitions used to perform the object-relational mapping with SQLAlchemy.
This is the basis for the data access layer too.
"""
...
class EntryPropertyAssociation(sql.Mappable):
    ...
    # field mapping
    entry_id = Column('entry_id', String, ForeignKey('Entries.entry_id', ondelete='CASCADE'), primary_key=True)
    prop_id = Column('prop_id', String, ForeignKey('Properties.prop_id', ondelete='CASCADE'), primary_key=True)
...
class EntryLanguagePropertyAssociation(sql.Mappable):
    """Association class between the EntryLanguageAssociation and Properties, used to capture language level properties.
    """
    # name of the corresponding table
    __tablename__ = 'EntryLanguageAssocPropertyAssoc'
    # field mapping
    ela_id = Column(String, ForeignKey('EntryLanguageAssoc.ela_id', ondelete='CASCADE'), primary_key=True)
    prop_id = Column(String, ForeignKey('Properties.prop_id', ondelete='CASCADE'), primary_key=True)
    value = Column(String, nullable=False)


class Property(sql.Mappable):
    """Representation of a property in the termbase structure, which may refer to entries, languages or single terms
    according to their level. Properties can be picklists, textual or images.
    """
    # name of the corresponding table
...
class Term(sql.Mappable):
    ...
    lang_id = Column(String, ForeignKey('Languages.locale', ondelete='CASCADE'), nullable=False,)
    vedette = Column(Boolean, nullable=False)
    entry_id = Column(String, ForeignKey('Entries.entry_id', ondelete='CASCADE'), nullable=False)
...
class TermPropertyAssociation(sql.Mappable):
    ...
    # field mapping
    term_id = Column(String, ForeignKey('Terms.term_id', ondelete='CASCADE'), primary_key=True)
    prop_id = Column(String, ForeignKey('Properties.prop_id', ondelete='CASCADE'), primary_key=True)
    value = Column(String, nullable=False)
